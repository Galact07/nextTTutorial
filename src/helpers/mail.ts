    ...
    try{
        ...
        var transporter = nodemailer.createTransport({
          ...
          const sender = await transporter.sendMail({
            ...
            html: `<p>Click on the url to <a href="${process.env.DOMAIN}/${emailTYPE == "VERIFY" ?`verifyMail?token=${token}`:`forgotPassword?token=${token}`}">Link</a> to 
            ${emailTYPE == "VERIFY" ? "verify your email " : "reset password "}</p> <br/>
            "${process.env.DOMAIN}/verifyMail?token=${token}"</br>
